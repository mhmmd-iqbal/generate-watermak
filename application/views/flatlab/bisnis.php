          ...
          <div class="panel-body">
            ...
            <div class="table-responsive">
              ...
              <tbody id="hasil-tabel">
                <?php $no=1; foreach($bisnis as $d ): ?>
                <tr>
                  <td><?=$no++?></td>
                  <td><?=$d['nama_bisnis']?></td>
                  <td>
                    <button class="btn btn-danger btn-sm hapus" value="<?=$d['id_bisnis']?>"><i class="fa fa-times"></i></button>
                    <button class="btn btn-primary btn-sm edit" nama="<?=$d['nama_bisnis']?>" value="<?=$d['id_bisnis']?>"><i class="fa fa-pencil"></i></button>
                  </td>
                </tr>
                <?php endforeach; ?>
              </tbody>
    ...
    <div class="modal-content">
      <div class="modal-body">
        <div class="row" style="padding-top: 20px">
          ...
          <div class="col-md-9">
            <input type="text" id="bisnis" class="form-control" name="">
          </div>
        </div>
      </div>
...
<script type="text/javascript">
  $('#simpan').click(function() {
    $.ajax({
    url : '<?=site_url('Action/data_bisnis/add')?>',
    type: 'POST',
    dataType: 'json',
    data: {nama_bisnis: $('#bisnis').val()},
  ...
  $(".hapus").click(function() {
    ...
    swal({ 
      ...
      switch(value){
        case "Hapus":
          $.ajax({
            url     : '<?=site_url('Action/data_bisnis/del/')?>'+id,
            type    : 'GET',
            dataType: 'json',
            success : function(data){
              swal({ 
  ...
  $(".edit").click(function() {
    ...
    $("#submit-edit").click(function() {
      var nama = $('#bisnis').val();
      $.ajax({
        url: '<?=site_url('Action/data_bisnis/update/')?>'+id,
        type: 'POST',
        dataType: 'json',
        data: {nama_bisnis: nama},
        success: function(data){
          swal({ 
